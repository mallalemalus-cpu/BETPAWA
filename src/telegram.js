...
function notifyNewBet(bet) {
  ...
  sels.forEach(function(s,i) {
    ...
    // Marché + cote + indicateur si estimée
    var estNote   = (s.justification && s.justification.includes("[cote estimée]")) ? " ⚠️<i>estimée</i>" : "";
    var learnNote = s.isLearning ? " 📚<i>apprentissage</i>" : "";
    lines.push("   → "+s.market+": <b>"+s.outcome+"</b> @ <b>"+s.odd+"</b>"+estNote+learnNote);
    if (s.justification && s.justification !== "Fallback v7" && s.justification !== "Fallback v5") {
      var cleanJust = s.justification.replace("[cote estimée]","").trim();
