...
function notifyNewBet(bet) {
  ...
  var lines = [];
  var phaseLabel = bet.phase ? " | Phase: "+bet.phase : "";
  lines.push("🎯 <b>NOUVEAU PARI — Cycle #"+bet.cycleNum+"</b>"+phaseLabel);
  lines.push("━━━━━━━━━━━━━━━━━━━━━━");
  ...
  sels.forEach(function(s,i) {
    // Championnat
    var league = s.league ? "🏆 "+s.league : "";
    // Date et heure du match
    var matchTime = "";
    if (s.datetime) {
      try {
        var d = new Date(s.datetime);
        // Format: Lun 24/03 à 20h45
        var days = ["Dim","Lun","Mar","Mer","Jeu","Ven","Sam"];
        var day  = days[d.getUTCDay()];
        var date = ("0"+d.getUTCDate()).slice(-2)+"/"+("0"+(d.getUTCMonth()+1)).slice(-2);
        var hour = ("0"+d.getUTCHours()).slice(-2)+"h"+("0"+d.getUTCMinutes()).slice(-2)+" UTC";
        matchTime = "📅 "+day+" "+date+" à "+hour;
      } catch {}
    }
    lines.push((i+1)+". <b>"+s.home+" vs "+s.away+"</b>");
    if (league)     lines.push("   "+league);
    if (matchTime)  lines.push("   "+matchTime);
    // Tier + Marché + cote
    var tierEmoji = s.tier==="S" ? "🌟" : s.tier==="A" ? "🔥" : s.tier==="B" ? "✅" : "📚";
    var estNote   = (s.justification && s.justification.includes("[cote estimée]")) ? " ⚠️est." : "";
    var learnNote = s.isLearning ? " 📚<i>apprent.</i>" : "";
    var tierLabel = s.tier ? " <b>["+s.tier+"]</b>" : "";
    lines.push("   → "+tierEmoji+tierLabel+" "+s.market+": <b>"+s.outcome+"</b> @ <b>"+s.odd+"</b>"+estNote+learnNote);
    // Biais et arbitrage détectés
    var metaInfo = [];
    if (s.biasDetected && s.biasType)  metaInfo.push("📉 Biais "+s.biasType);
    if (s.arbitrageDetected)           metaInfo.push("⚖️ Arb. implicite");
    if (metaInfo.length > 0) lines.push("   "+metaInfo.join(" | "));
    // Justification courte
    if (s.justification && s.justification !== "Fallback v7" && s.justification !== "Fallback v5") {
      var cleanJust = s.justification.replace("[cote estimée]","").replace("[apprentissage]","").trim();
      if (cleanJust.length > 5) lines.push("   💡 "+cleanJust.slice(0,80));
    }
  });
  ...
  if (sels.length > 0) {
    ...
    sels.forEach(function(s){ if(s.tier) tierCounts[s.tier]++; });
    var biasCount = sels.filter(function(s){return s.biasDetected;}).length;
    var arbCount  = sels.filter(function(s){return s.arbitrageDetected;}).length;
    var tierSummary = ["S:"+tierCounts.S,"A:"+tierCounts.A,"B:"+tierCounts.B,"C:"+tierCounts.C].join(" | ");
    lines.push("📊 Tiers: "+tierSummary+(biasCount>0?" | Biais: "+biasCount:"")+(arbCount>0?" | Arb: "+arbCount:""));
    // EV du ticket global si disponible
    if (bet.ticketEV !== undefined && bet.ticketEV !== null) {
      var evLabel = bet.ticketEV >= 0 ? "+"+bet.ticketEV.toFixed(3) : bet.ticketEV.toFixed(3);
      lines.push("💰 EV ticket: <b>"+evLabel+" FCFA</b>");
    }
  }
  lines.push("━━━━━━━━━━━━━━━━━━━━━━");
