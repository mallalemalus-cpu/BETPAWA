...
function loadMemory()   {
  try {
    if(!fs.existsSync(MEMORY_FILE)) return JSON.parse(JSON.stringify(DEFAULT_MEMORY));
    var m=JSON.parse(fs.readFileSync(MEMORY_FILE,"utf8"));
    var result=Object.assign({},JSON.parse(JSON.stringify(DEFAULT_MEMORY)),m);
    // Migration v10: s'assurer des bonnes valeurs
    if(result.dynamicParams.maxEventsPerTicket<18) result.dynamicParams.maxEventsPerTicket=18;
    if(result.dynamicParams.minEventsPerTicket<8)  result.dynamicParams.minEventsPerTicket=8;
    if(result.dynamicParams.maxSingleOdd>10)       result.dynamicParams.maxSingleOdd=6.0;
    return result;
  } catch { return JSON.parse(JSON.stringify(DEFAULT_MEMORY)); }
}
