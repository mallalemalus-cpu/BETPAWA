...
// ─── MODULE 13: SCORING BOOKMAKER /10 ────────────────────────────────────────
// Objectif: identifier les cotes mal ajustées (value cachée)
// Comparer la cote proposée à la cote consensus du marché
// Score /10 : +2 (écart >5%), +3 (>10%), +5 (>15%)
function scoreBookmaker(offeredOdd, marketConsensusOdd) {
  if (!offeredOdd || !marketConsensusOdd || marketConsensusOdd <= 0) return { score: 0, ecart: 0, isValue: false };
  var offered  = parseFloat(offeredOdd);
  var consensus = parseFloat(marketConsensusOdd);
  // Écart relatif: si offeredOdd > consensus → value potentielle
  var ecartPct = parseFloat(((offered - consensus) / consensus * 100).toFixed(4));
  var score = 0;
  if (ecartPct >= 15) score = 5;
  else if (ecartPct >= 10) score = 3;
  else if (ecartPct >= 5)  score = 2;
  // Bonus: score bookmaker de référence (Pinnacle = sharp)
  var bonusSharp = consensus > 0 && offered > consensus * 1.03 ? 1 : 0;
  score = Math.min(10, score + bonusSharp);
  return {
    score:       score,
    scoreLabel:  score >= 7 ? "EXCELLENT" : score >= 5 ? "EXPLOITABLE" : score >= 3 ? "FAIBLE" : "IGNORÉ",
    ecartPct:    parseFloat(ecartPct.toFixed(2)),
    offeredOdd:  offered,
    consensus:   consensus,
    isValue:     score >= 5 && ecartPct > 0,
    isExploitable: score >= 5,
  };
}

// Calculer le consensus de marché pour une sélection donnée (h2h, btts, totals...)
function computeMarketConsensus(multiOddsData, market, outcome) {
  if (!multiOddsData || !multiOddsData.length) return null;
  var values = [];
  multiOddsData.forEach(function(bk) {
    if (!bk[market]) return;
    var val = bk[market][outcome];
    if (val && isFinite(parseFloat(val))) values.push(parseFloat(val));
  });
  if (values.length < 2) return null;
  // Moyenne harmonique (utilisée par Pinnacle pour les cotes)
  var harmonic = values.length / values.reduce(function(s,v){ return s + 1/v; }, 0);
  return parseFloat(harmonic.toFixed(3));
}

// Score global d'un match selon tous ses marchés
function scoreMatchBookmakers(match, multiOddsData) {
  var odds = match.odds || {};
  var scores = {};
  var consensus = {
    home: computeMarketConsensus(multiOddsData, "h2h", "home"),
    draw: computeMarketConsensus(multiOddsData, "h2h", "draw"),
    away: computeMarketConsensus(multiOddsData, "h2h", "away"),
    over25: computeMarketConsensus(multiOddsData, "totals", "over25"),
    btts_yes: computeMarketConsensus(multiOddsData, "btts", "btts_yes"),
  };
  if (odds.home && consensus.home)    scores.home    = scoreBookmaker(odds.home, consensus.home);
  if (odds.draw && consensus.draw)    scores.draw    = scoreBookmaker(odds.draw, consensus.draw);
  if (odds.away && consensus.away)    scores.away    = scoreBookmaker(odds.away, consensus.away);
  if (odds.over25 && consensus.over25) scores.over25  = scoreBookmaker(odds.over25, consensus.over25);
  if (odds.btts_yes && consensus.btts_yes) scores.btts = scoreBookmaker(odds.btts_yes, consensus.btts_yes);
  // Score max = meilleure opportunité sur ce match
  var maxScore = 0;
  Object.values(scores).forEach(function(s){ if(s.score > maxScore) maxScore = s.score; });
  return { scores: scores, maxScore: maxScore, consensus: consensus };
}

// ─── MODULE 14: CLV COMPLET ──────────────────────────────────────────────────
// CLV = (cote_prise - cote_finale) / cote_finale * 100
// CLV positif = bon pari (marché te donne raison)
// CLV négatif = mauvais timing ou mauvaise lecture
function trackCLV(selection, closingOdds) {
  ...
  var clvPct     = ((betOdd - closingOdd) / closingOdd) * 100; // formule exacte du module 14
  ...
  return {
    ...
    interpretation: clvPct > 0
      ? "Cote prise avant correction marché → edge confirmé"
      : clvPct > -3
        ? "Légère sous-performance timing — acceptable"
        : "Marché plus précis → revoir timing de prise",
  };
}

// Calculer le CLV moyen d'un historique de paris
function computeAverageCLV(bets) {
  var clvValues = [];
  (bets || []).forEach(function(b) {
    (b.selections || []).forEach(function(s) {
      if (s.clv !== undefined && s.clv !== null) clvValues.push(s.clv);
      // Estimation si cote finale non encore disponible: supposer -2% (ajustement marché typique)
      else if (s.odd && b.status === "resolved") clvValues.push(-1.5);
    });
  });
  if (!clvValues.length) return null;
  var avg = clvValues.reduce(function(s,v){return s+v;},0) / clvValues.length;
  return {
    average:  parseFloat(avg.toFixed(2)),
    count:    clvValues.length,
    positive: clvValues.filter(function(v){return v>0;}).length,
    negative: clvValues.filter(function(v){return v<=0;}).length,
    rating:   avg > 3 ? "EXCELLENT" : avg > 0 ? "BON" : avg > -3 ? "NEUTRE" : "NÉGATIF",
    interpretation: avg > 0
      ? "Timing de prise de paris au-dessus du marché"
      : "Timing à améliorer — parier plus tôt après parution des cotes",
  };
}
