...
const { runSelfEvaluation, validateReasoningChain, autoCorrectParams, COGNITIVE_BIASES } = require("./selfHealer");
...
const CONFIG_BASE = {
  ...
  TARGET_SELECTIONS:     6,     // Nombre optimal de sélections pour range 30-400
...
async function runCycle() {
  ...
  try {
    ...
    logger.info("✅ "+matches.length+" matchs disponibles");
  } catch(e) {
    logger.error("❌ "+e.message);
    await telegram.notifyAlert("erreur","Matchs indisponibles: "+e.message).catch(function(){});
    return null;
  }
  if (!matches.length) { logger.warn("⚠️ Aucun match"); return null; }
  ...
  var headlines = "";
  try {
    headlines = await extractGlobalHeadlines();
    // Rechercher sur les 6 premiers matchs
    for (var ri=0; ri<Math.min(matches.length,6); ri++) {
      var m = matches[ri];
      var mr = await conductFullResearch(m.home, m.away, m.league, m.datetime);
      if (mr && mr.newsItems && mr.newsItems.length) {
        researchResults.matchResearch.push(Object.assign({ home:m.home, away:m.away }, mr));
        researchResults.newsItems = researchResults.newsItems.concat(mr.newsItems||[]);
        researchResults.injuries  = researchResults.injuries.concat(mr.injuries||[]);
      }
    }
  ...
  if (resolved.length >= 2) {
    ...
    catch(e){ logger.warn("Critique: "+e.message); }
  }
  ...
  var mise = agentDecision.forcedMinMise
    ? Math.max(100, Math.floor(stats.bankroll*0.005))
    ...
    try {
      ...
      for (var si=0; si<(bet.selections||[]).length; si++) {
        ...
        if (!sel.matchId||String(sel.matchId).startsWith("SIM_")) {
          var sim2 = simulateResultFromOdd(sel.odd);
          sel.won=sim2.won; sel.actualResult=sim2.outcome;
          if(!sim2.won)betWon=false; continue;
        }
...
function calculateMise(bankroll, totalOdd, confidence, ctxAnalyses, ticketSim) {
  if(!bankroll||bankroll<=0)return 100;
  ...
  return Math.max(100,Math.min(Math.floor(mise*mult),Math.floor(bankroll*0.05)));
}
