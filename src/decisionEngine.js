...
function buildOptimalTicket(matches, contextAnalyses, sharpAnalyses, dynCfg, history) {
  ...
  for (var i = 0; i < validatedMatches.length; i++) {
    ...
    var validCands = analysis.candidates.filter(function(c) {
      return blackMkt.indexOf(c.market) < 0 &&
             c.offeredOdd >= 1.15 &&
             c.offeredOdd <= maxOdd &&
             c.edge >= MIN_EV_THRESHOLD; // FILTRE EV RÉEL: edge >= +2%
    });
  ...
  // Passe principale: sélection avec contraintes de diversification
...
function validateAndDeduplicateMatches(matches) {
  // 1. Dédupliquer par home_away_date (multi-sources)
  var seen = {};
  var deduped = [];
  matches.forEach(function(m) {
    var date = m.datetime ? m.datetime.slice(0, 10) : "unknown";
    var key  = (m.home + "_" + m.away + "_" + date).toLowerCase().replace(/\s/g, "");
  ...
  return deduped.filter(function(m) {
    var date  = m.datetime ? m.datetime.slice(0, 10) : "unknown";
    var key1  = (m.home + "_" + date).toLowerCase();
    var key2  = (m.away + "_" + date).toLowerCase();
    if (teamsByDate[key1] || teamsByDate[key2]) return false;
    teamsByDate[key1] = true;
    teamsByDate[key2] = true;
    return true;
  });
}
