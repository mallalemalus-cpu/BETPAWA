...
function getDashboard(stats, bets, memory, scores) {
  ...
  // Calcul CLV moyen
  var clvAvgDisplay = "N/A";
  var clvColor = "var(--muted)";
  try {
    var clvVals = [];
    resolved.forEach(function(b){ (b.selections||[]).forEach(function(s){ if(s.clv !== undefined) clvVals.push(s.clv); }); });
    if (clvVals.length >= 3) {
      var clvM = (clvVals.reduce(function(s,v){return s+v;},0)/clvVals.length).toFixed(1);
      clvAvgDisplay = clvM + "% (" + clvVals.length + " paris)";
      clvColor = parseFloat(clvM)>=0 ? "var(--green)" : "var(--red)";
    }
  } catch {}

  // Top ligues rentables
  var leagueStats = {};
  resolved.forEach(function(b){ (b.selections||[]).forEach(function(s){
    var lg = s.league||"N/A";
    if(!leagueStats[lg]) leagueStats[lg]={gain:0,total:0};
    leagueStats[lg].total++;
    leagueStats[lg].gain += b.won?(b.gainNet||0):-(b.mise||1);
  }); });
  var topLeaguesHtml = Object.keys(leagueStats)
    .filter(function(lg){ return leagueStats[lg].total>=3; })
    .sort(function(a,b){ return leagueStats[b].gain-leagueStats[a].gain; })
    .slice(0,5).map(function(lg){
      var g = leagueStats[lg].gain;
      var c = g>=0?"var(--green)":"var(--red)";
      return '<div style="display:flex;justify-content:space-between;padding:3px 0;font-size:10px"><span style="color:var(--muted)">'+lg+'</span><span style="color:'+c+'">'+(g>=0?"+":"")+g+'F ('+leagueStats[lg].total+')</span></div>';
    }).join("") || '<div style="color:var(--muted);font-size:10px">Pas encore de données</div>';

  // Détection drawdown
  var drawdown = 0, peak = stats.bankroll||5000;
  resolved.slice().reverse().forEach(function(b){
    var val = b.won?(b.gainNet||0):-(b.mise||0);
    if(val > 0) peak = Math.max(peak, (stats.bankroll||5000)+val);
  });
  drawdown = parseFloat(((peak-(stats.bankroll||5000))/peak*100).toFixed(1));
...
<div class="page active" id="page-overview">
  ...
  <!-- M16: KPIs CLV + Drawdown -->
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:14px;">
    <div class="kpi">
      <div class="kl">CLV MOYEN</div>
      <div class="kv" style="color:${clvColor};font-size:18px">${clvAvgDisplay}</div>
      <div class="ks">Timing de prise</div>
    </div>
    <div class="kpi">
      <div class="kl">DRAWDOWN</div>
      <div class="kv" style="color:${drawdown>10?"var(--red)":drawdown>5?"var(--orange)":"var(--green)"};font-size:18px">${drawdown}%</div>
      <div class="ks">Par rapport au pic</div>
    </div>
  </div>

  <!-- M16: Top Ligues -->
  <div class="sh">Top Ligues Rentables</div>
  <div style="background:var(--bg2);border:1px solid var(--border);border-radius:6px;padding:10px;margin-bottom:14px">
    ${topLeaguesHtml}
  </div>
