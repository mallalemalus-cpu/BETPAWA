...
async function fetchUpcomingMatches() {
  ...
  logger.warn("Fallback données simulées réalistes");
...
async function fetchFromOddsAPI() {
  ...
  for (var i = 0; i < Math.min(FOOTBALL_LEAGUES.length, 5); i++) {
    ...
    try {
      var url = "https://api.the-odds-api.com/v4/sports/"+league+"/odds/?apiKey="+ODDS_API_KEY+"&regions=eu&markets=h2h,totals,btts&oddsFormat=decimal&dateFormat=iso";
    ...
    try {
      ...
      (data.matches||[]).forEach(function(m){
        all.push({
          ...
          datetime: m.utcDate,
          odds: generateOdds(m.homeTeam&&m.homeTeam.name, m.awayTeam&&m.awayTeam.name),
...
module.exports = { fetchUpcomingMatches, fetchMatchResult, fetchWeather, fetchTeamStats };
