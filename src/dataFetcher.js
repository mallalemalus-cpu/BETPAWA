...
async function fetchUpcomingMatches() {
  ...
  if (ODDS_API_KEY) {
    try {
      ...
      logger.info("Source: The Odds API — " + matches.length + " matchs bruts");
      if (matches.length >= 15) {
        // Fusionner ensuite avec football-data si disponible pour enrichir les IDs
      } else {
        // Pas assez → essayer football-data en plus
      }
  ...
  if (FOOTBALL_DATA_KEY) {
    try {
      ...
      // Merge robuste: dédupliquer par noms normalisés (gère les variantes de noms)
      fd.forEach(function(m){
        var norm = normalizeTeamKey(m.home, m.away, m.datetime);
        var dup = matches.some(function(ex){ return normalizeTeamKey(ex.home, ex.away, ex.datetime) === norm; });
        if (!dup) matches.push(m);
      });
  ...
  if (matches.length === 0) {
    if (ODDS_API_KEY || FOOTBALL_DATA_KEY) {
      logger.warn("⚠️ APIs configurées mais indisponibles — cycle annulé");
      return [];
    }
  ...
  // ─── FILTRE TEMPOREL: uniquement les matchs du prochain cycle (6-12h) ────
  var now = Date.now();
  var windowStart = now;                    // maintenant
  var windowEnd   = now + 12 * 3600 * 1000; // +12h (couvre le prochain cycle)

  var inWindow = matches.filter(function(m) {
    if (!m.datetime) return true; // garder si pas de datetime
    var t = new Date(m.datetime).getTime();
    return t >= windowStart && t <= windowEnd;
  });

  // Si aucun match dans la fenêtre → prendre les 24h suivantes (day off)
  if (inWindow.length < 5 && matches.length >= 5) {
    var extended = matches.filter(function(m) {
      if (!m.datetime) return true;
      var t = new Date(m.datetime).getTime();
      return t >= windowStart && t <= now + 24 * 3600 * 1000;
    });
    if (extended.length >= 5) {
      logger.info("⏰ Fenêtre étendue à 24h: " + extended.length + " matchs");
      return extended;
    }
    logger.info("⏰ Peu de matchs ce cycle — " + matches.length + " matchs totaux retenus");
    return matches; // garder tous si vraiment peu
  }

  logger.info("⏰ Fenêtre 12h: " + inWindow.length + "/" + matches.length + " matchs retenus");
  return inWindow;
}

// Normalise une paire home/away en clé unique pour la déduplication
// Gère les variantes: "Crystal Palace" = "Crystal Palace F.C." = "Crystal Palace FC"
function normalizeTeamName2(name) {
  if (!name) return "";
...
function normalizeTeamKey(home, away, datetime) {
  var date = datetime ? datetime.slice(0, 10) : "unknown";
  return normalizeTeamName2(home) + "_" + normalizeTeamName2(away) + "_" + date;
}
...
async function fetchFromOddsAPI() {
  ...
  for (var i = 0; i < Math.min(FOOTBALL_LEAGUES.length, 5); i++) {
    ...
    try {
      // btts est supporté sur le plan gratuit The Odds API
      // L'erreur 422 précédente venait de la combinaison btts+totals+h2h simultanée
      // On fait 2 appels séparés: d'abord h2h+totals, puis btts si quota restant
...
function parseOddsEvent(ev, league) {
  try {
    ...
    // BTTS: utiliser la cote réelle si fournie par l'API
    // Si absente (plan gratuit Odds API ne fournit pas btts):
    // → Estimer via Poisson MAIS marquer comme estimée
    if (!odds.btts_yes || !odds.btts_no) {
      var bttsProb = estimateBTTS(odds);
      if (bttsProb > 0) {
        // Calibrage réaliste: BTTS OUI sur de vrais bookmakers est entre 1.60 et 2.05
        // Pas 2.22-2.27 (erreur du modèle précédent)
        var rawBttsYes = 1 / (bttsProb * 0.94);
        var rawBttsNo  = 1 / ((1 - bttsProb) * 0.94);
        // Clipper dans les ranges réalistes du marché
        odds.btts_yes = parseFloat(Math.min(2.10, Math.max(1.55, rawBttsYes)).toFixed(2));
        odds.btts_no  = parseFloat(Math.min(3.50, Math.max(1.55, rawBttsNo)).toFixed(2));
        odds.btts_estimated = true; // marquage: cote estimée (pas API réelle)
      }
    }
    ...
    return { id:ev.id, home:ev.home_team, away:ev.away_team, league:formatLeague(league), datetime:ev.commence_time, odds:odds, source:"odds_api", isReal:true };
  } catch { return null; }
}
    ...
    try {
      ...
      (data.matches||[]).forEach(function(m){
        all.push({
          ...
          isReal: true, // match réel football-data.org
        });
      });
      await sleep(250);
...
function generateRealisticMatches() {
  ...
  return fixtures.map(function(f, i) {
    ...
    return {
      ...
      source: "simulated",
      isReal: false, // ATTENTION: match simulé pour mode démo uniquement
    };
  });
}
...
function enrichOdds(odds) {
  if (!odds) return odds;
  if (!odds.btts_yes || !odds.btts_no) {
    var bttsProb2 = estimateBTTS(odds);
    if (bttsProb2 > 0) {
      var rawY2 = 1 / (bttsProb2 * 0.94);
      var rawN2 = 1 / ((1 - bttsProb2) * 0.94);
      odds.btts_yes = parseFloat(Math.min(2.10, Math.max(1.55, rawY2)).toFixed(2));
      odds.btts_no  = parseFloat(Math.min(3.50, Math.max(1.55, rawN2)).toFixed(2));
      odds.btts_estimated = true;
    }
  }
  if (!odds.dc_1X && odds.home && odds.draw) {
    var p1X = (1/parseFloat(odds.home)) + (1/parseFloat(odds.draw));
