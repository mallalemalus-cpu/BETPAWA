...
async function fetchClubEloRating(teamName) {
  try {
    ...
    var url = "http://clubelo.com/"+cleanName+"/json";
    var data = await fetchURL(url, {}, true);
    var parsed = JSON.parse(data);
    if (parsed && parsed.length > 0) {
      var latest = parsed[parsed.length-1];
      return {
        team: teamName,
        elo: parseFloat(latest.Elo||latest.elo||0),
        rank: parseInt(latest.Rank||latest.rank||999),
        country: latest.Country||latest.country||"",
        level: parseInt(latest.Level||latest.level||1),
        source: "clubelo.com",
      };
    }
    return null;
